...
```bash
conda create -n semantic_layer_data_preparation python=3.9
conda activate semantic_layer_data_preparation
```
...
```
This command will create your personal repository under `/Repos/<username>/semantic_layer_data_preparation`.
3. To set up the CI/CD pipeline with the notebook, create a separate `Staging` repo:
```bash
databricks repos create --url <your repo URL> --provider <your-provider> --path /Repos/Staging/semantic_layer_data_preparation
```
...
- To trigger the release pipeline, get the current version from the `semantic_layer_data_preparation/__init__.py` file and tag the current code version:
```
